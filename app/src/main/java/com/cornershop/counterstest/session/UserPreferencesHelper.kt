...
class UserPreferencesHelper (
    ...
    private var sharedPreferences: SharedPreferences? = context.getSharedPreferences(SHARED_PREFERENCES_USER_PREFERENCES_PATH, Context.MODE_PRIVATE)
    ...
    override fun hasUserSeenWelcomeScreenBefore(): Boolean {
        return sharedPreferences?.getBoolean(SHARED_PREFERENCES_KEY_USER_HAS_SEEN_WELCOME_SCREEN, false) ?: false
    }

    override fun setHasUserSeenWelcomeScreenBefore(hasSeenIt: Boolean) {
        sharedPreferences?.edit()?.putBoolean(SHARED_PREFERENCES_KEY_USER_HAS_SEEN_WELCOME_SCREEN, hasSeenIt)?.apply()
    }
